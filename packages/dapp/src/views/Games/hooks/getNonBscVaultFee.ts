...
export const getNonBscVaultContractFee = async ({
  ...
  try {
    ...
    const getNonce = await crossGamingAddress.nonces(userAddress, pid)
    ...
    const [encodeMessage, hasFirstTime, estimateGaslimit] = await Promise.all([
      ...
      crossGamingAddress.is1st(userAddress),
      crossGamingAddress.estimateGaslimit(Chains.BSC, userAddress, messageType),
    ])
    ...
    if (messageType >= MessageTypes.Withdraw) {
      const estimateEvmGaslimit = await crossGamingAddress.estimateGaslimit(Chains.EVM, userAddress, messageType)
