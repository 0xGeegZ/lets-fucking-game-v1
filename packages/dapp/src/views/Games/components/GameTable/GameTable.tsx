...
import { DesktopColumnSchema, GameWithStakedValue } from '../types'

export interface ITableProps {
  farms: GameWithStakedValue[]
...
const GameTable: React.FC<React.PropsWithChildren<ITableProps>> = ({ farms, cakePrice, userDataReady }) => {
      ...
      <TableContainer id="table-container">
        <TableWrapper ref={tableWrapperEl}>
          <StyledTable>
            <TableBody>
              {sortedRows.map((row) => {
                return <Row {...row} userDataReady={userDataReady} key={`table-row-${row.game.pid}`} />
                // row?.details?.boosted ? (
                //   <ProxyFarmContainer key={`table-row-${row.game.pid}`} farm={row.details}>
                //     <Row {...row} userDataReady={userDataReady} />
                //   </ProxyFarmContainer>
                // ) : (
                //   <Row {...row} userDataReady={userDataReady} key={`table-row-${row.game.pid}`} />
                // )
              })}
            </TableBody>
          </StyledTable>
