...
import { fetchGameUserDataAsync } from 'state/games'
...
import useApproveGame from '../../../hooks/useApproveGame'
import useStakeGames from '../../../hooks/useStakeGames'
import useUnstakeGames from '../../../hooks/useUnstakeGames'
...
interface StackedActionProps extends GameWithStakedValue {
...
export function useStakedActions(lpContract, pid, vaultPid) {
  ...
  const { onStake } = useStakeGames(pid, vaultPid)
  const { onUnstake } = useUnstakeGames(pid, vaultPid)
  const dispatch = useAppDispatch()

  const { onApprove } = useApproveGame(lpContract, chainId)

  const onDone = useCallback(
    () => dispatch(fetchGameUserDataAsync({ account, pids: [pid], chainId })),
      ...
      if (receipt?.status) {
        toastSuccess(
          ...
          <ToastDescriptionWithTx txHash={receipt.transactionHash}>
            {t('Your funds have been staked in the game')}
