...
import MultiChainHarvestModal from 'views/Games/components/MultiChainHarvestModal'
import useProxyStakedActions from 'views/Farms/components/YieldBooster/hooks/useProxyStakedActions'
import useHarvestGame from '../../../hooks/useHarvestGame'
import { GameWithStakedValue } from '../../types'
import { ActionContainer, ActionContent, ActionTitles } from './styles'

interface HarvestActionProps extends GameWithStakedValue {
...
export const HarvestActionContainer = ({ children, ...props }) => {
  const { onReward } = useHarvestGame(props.pid)
  const { account, chainId } = useActiveWeb3React()
  const dispatch = useAppDispatch()

  const onDone = useCallback(
    () => dispatch(fetchGameUserDataAsync({ account, pids: [props.pid], chainId })),
...
export const HarvestAction: React.FunctionComponent<React.PropsWithChildren<HarvestActionProps>> = ({
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    `${toolTipBalance} ${t(
      `CAKE has been harvested to the game booster contract and will be automatically sent to your wallet upon the next harvest.`,
    )}`,
    {
      placement: 'bottom',
    },
