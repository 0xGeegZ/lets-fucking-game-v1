...
const StakedLP: React.FunctionComponent<React.PropsWithChildren<StakedLPProps>> = ({
  ...
  const onClickLoadingIcon = () => {
    const { length } = pendingGame
    if (length) {
      if (length > 1) {
        onPresentTransactionModal()
      } else {
        dispatch(pickFarmTransactionTx({ tx: pendingGame[0].txid, chainId }))
      }
    }
  }
  ...
  return (
    <Flex flexDirection="column" alignItems="flex-start">
      <Flex>
        ...
        {pendingGame.length > 0 && <RefreshIcon style={{ cursor: 'pointer' }} spin onClick={onClickLoadingIcon} />}
      </Flex>
