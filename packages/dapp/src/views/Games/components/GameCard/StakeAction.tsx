...
const StakeAction: React.FC<React.PropsWithChildren<GameCardActionsProps>> = ({
  pid,
  vaultPid,
  quoteToken,
  token,
  ...
  const pendingGame = useNonBscFarmPendingTransaction(lpAddress)

  const isStakeReady = useMemo(() => {
    return ['history', 'archived'].some((item) => router.pathname.includes(item)) || pendingGame.length > 0
  }, [pendingGame, router])
  ...
  const handleNonBscStake = async (amountValue: string) => {
    ...
    const [receipt, isFirstTime] = await Promise.all([
      ...
      crossGamingAddress.is1st(account),
    ])
  ...
  const renderStakingButtons = () => {
    return stakedBalance.eq(0) ? (
      ...
      <IconButtonWrapper>
        <IconButton mr="6px" variant="tertiary" disabled={pendingGame.length > 0} onClick={onPresentWithdraw}>
