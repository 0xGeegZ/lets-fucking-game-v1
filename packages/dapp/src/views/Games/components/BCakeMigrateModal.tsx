...
export const BCakeMigrateModal: React.FC<BCakeMigrateModalProps> = ({
  ...
  const migrationSteps: Record<Steps, string> = useMemo(
    () => ({
      [Steps.Unstake]: t('Unstake LP tokens from the game'),
      [Steps.Enable]: t('Enable staking with yield booster'),
      [Steps.Stake]: t('Stake LP tokens back to the game'),
    }),
    [t],
  )
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    <Text>
      {t(
        'To enable game yield boosters, you must follow the guide and migrate your current gameing positions. However, for each game, you will only need to migrate once.',
      )}
    </Text>,
    {
      placement: 'right',
  ...
  return (
    <Modal title={t('Migrate your stakings')} width="420px" onDismiss={onDismiss}>
      ...
      <InfoBox ref={targetRef}>
        <InfoText>{t('You will need to migrate your stakings before activating yield booster for a game')}</InfoText>
        <InfoIconBox>
          <HelpIcon />
        </InfoIconBox>
      </InfoBox>
      ...
      <FooterBox>
        <Text mb="16px" textAlign="center">
          ...
          {/* t('Unstake LP tokens from the game') */}
          {/* t('Enable staking with yield booster') */}
          {/* t('Stake LP tokens back to the game') */}
        </Text>
        <Button
