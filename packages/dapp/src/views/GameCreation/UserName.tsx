...
import ConfirmGameCreationModal from './ConfirmGameCreationModal'
...
const UserName: React.FC<React.PropsWithChildren> = () => {
  ...
  const { teamId, selectedNft, userName, actions, minimumCakeRequired, allowance } = useGameCreation()
  ...
  const [onPresentConfirmGameCreation] = useModal(
    <ConfirmGameCreationModal
      userName={userName}
  ...
  return (
    <>
      ...
      <Button onClick={onPresentConfirmGameCreation} disabled={!isValid || !isUserCreated} id="completeGameCreation">
        {t('Complete Profile')}
      </Button>
