...
const saveContract = ({ contractName, chainId, contractData }) => {
  ...
  if (!deployedContracts[chainId]) {
    deployedContracts[chainId] = {
      [contractName]: { ...contractData[chainId][contractName], olds: [], date: new Date().toISOString() },
    };
...
const saveContractData = (contractData) => {
  ...
  Object.keys(contractData[chainId]).map((contractName) => saveContract({ contractName, chainId, contractData }));
};
